...
const BusinessModel = () => {
  ...
  const presentationItems = [
    ...pricingTiers.map((tier, index) => ({
      ...
      description: `${tier.price} ${tier.period}`,
      content: (
        <div className="flex flex-col items-center justify-center h-full text-center space-y-8">
          <div className="space-y-4">
            <CreditCard className="h-20 w-20 mx-auto text-primary" />
            <div className="text-6xl font-bold">{tier.price}</div>
            <div className="text-2xl font-medium text-muted-foreground">{tier.name} Plan</div>
            <div className="text-lg text-muted-foreground">{tier.period}</div>
          </div>
    ...
    ...revenueProjections.map((projection, index) => ({
      id: `revenue-${index}`,
      title: `${projection.year} Revenue`,
      description: `$${projection.total}K ARR projected`,
      content: (
        <div className="flex flex-col items-center justify-center h-full text-center space-y-8">
          <div className="space-y-4">
            <TrendingUp className="h-20 w-20 mx-auto text-green-600" />
            <div className="text-6xl font-bold">${projection.total}K</div>
            <div className="text-2xl font-medium text-muted-foreground">{projection.year} ARR</div>
          </div>
          <div className="max-w-2xl space-y-4">
            <div className="grid grid-cols-3 gap-4">
              <div className="p-3 bg-blue-50 rounded-lg dark:bg-blue-950/20">
                <div className="text-lg font-semibold text-blue-600">${projection.individual}K</div>
                <div className="text-xs text-muted-foreground">Individual</div>
              </div>
              <div className="p-3 bg-green-50 rounded-lg dark:bg-green-950/20">
                <div className="text-lg font-semibold text-green-600">${projection.professional}K</div>
                <div className="text-xs text-muted-foreground">Professional</div>
              </div>
              <div className="p-3 bg-purple-50 rounded-lg dark:bg-purple-950/20">
                <div className="text-lg font-semibold text-purple-600">${projection.enterprise}K</div>
  ...
  const parentTileItems = [
    {
      ...
      description: 'Three-tier pricing model designed for different customer segments',
      content: (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {pricingTiers.map((tier, index) => (
              <ClickableTile 
                key={tier.name}
                onClick={() => openPresentation(`pricing-${index}`)}
                className={`relative p-6 rounded-lg border-2 ${tier.color}`}
                hoverScale={false}
              >
                {tier.popular && (
          ...
          <div className="text-center text-muted-foreground">
            Click on any pricing tier to view detailed presentation
          </div>
        </div>
      )
    },
    {
      ...
      description: '3-year revenue forecast by pricing tier (in thousands)',
      content: (
        <div className="space-y-6">
          {revenueProjections.map((projection, index) => (
            <ClickableTile 
              key={projection.year}
              onClick={() => openPresentation(`revenue-${index}`)}
          ...
          <div className="text-center text-muted-foreground">
            Click on any year to view detailed revenue presentation
          </div>
        </div>
      )
    },
    {
      ...
      description: 'Key financial metrics by customer segment',
      content: (
        <div className="space-y-6">
          ...
          <div className="text-center text-muted-foreground">
            Unit economics showing healthy LTV:CAC ratios across all segments
          </div>
        </div>
      )
    }
